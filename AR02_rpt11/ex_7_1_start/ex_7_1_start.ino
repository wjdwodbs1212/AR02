...
void loop(){
  // 포텐쇼미터 값을 읽어옴
  int potentioMeter1 = digitalRead(potentioMeterPin);
  if(digitalRead(inputPin2)==LOW) {
    motorSpeed = map(potentioMeter1,512+(stopRange/2),1023,4500,1000);
    motorSpeedPercent = map(motorSpeed,4500,1000,1,100);
    clockwise();
  }
  else if (digitalRead(inputPin4)==LOW ){
    motorSpeed = map(potentioMeter1,512-(stopRange/2),0,4500,1000);
    motorSpeedPercent = map(motorSpeed,4500,1000,1,100);
    counterClockwise();
  ...
  int potentioMeter2 = analogRead(potentioMeterPin);

   //CW로 회전시 모터를 CW방향으로 회전시킨다.
  if(potentioMeter2 >= 512+(stopRange/2)){
    //모터의 속도를 계산한다.
    motorSpeed = map(potentioMeter2,512+(stopRange/2),1023,4500,1000);
    // 모터의 속도를 백분율로 변환시킨다.
    motorSpeedPercent = map(motorSpeed,4500,1000,1,100);
    // 시리얼 통신 메세지를 출력한다.
  ...
  else if(potentioMeter2 <= 512-(stopRange/2)){
    //모터의 속도를 계산한다.  	
    motorSpeed = map(potentioMeter2,512-(stopRange/2),0,4500,1000);
     //모터의 속도를 백분율로 변환시킨다.
    motorSpeedPercent = map(motorSpeed,4500,1000,1,100);
    // 시리얼 통신 메세지를 출력한다.
